...
const JWT_SECRET =
  process.env.JWT_SECRET_KEY || process.env.JWT_SECRET || "secret-reset-key";
...
const resetPasswordSchema = z
  ...
  .refine(
    (data) => (data.newPassword || data.password) === data.confirmPassword,
    {
...
export const createUser = async (
  ...
  try {
    ...
    if (phone) {
      ...
      if (
        pendingPhoneRegistration[0]?.email !== undefined &&
    ...
    const verificationCodeExpiresAt = new Date(
      now.getTime() + VERIFICATION_CODE_EXPIRY_MS,
    );
    ...
    response.status(202).json({
      message:
        "Verification code sent to your email. Verify it to finish registration.",
    });
  } catch (error) {
    if (
    ...
    if (
      ...
      console.error(
        "Failed to start registration: database table does not exist",
      );
      response.status(503).json({
        message:
          "Database schema is not initialized. Run the Prisma migration first.",
      });
      return;
    }
...
export const verifyEmailAndCreateUser = async (
  ...
  try {
    const registrations = await prisma.$queryRaw<PendingRegistration[]>`
      SELECT
        "id", "name", "email", "phone", "passwordHash", "verificationCodeHash",
...
export const resendVerificationCode = async (
  ...
  try {
    ...
    const retryAfterSeconds = Math.ceil(
      (sentAtTime + RESEND_COOLDOWN_MS - now.getTime()) / 1000,
    );
...
export const resetPassword = async (
  ...
  try {
    ...
    try {
      decoded = jwt.verify(activeToken, JWT_SECRET) as {
        email?: string;
        userId?: string;
        purpose?: string;
      };
